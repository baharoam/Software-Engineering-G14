...
class ModuleManager extends Component
{
    ...
    public function getModules($options = [])
    {
        ...
        $includeCoreModules = $options['includeCoreModules'];
        $enabledOnly = $options['enabled'];
        $returnClass = $options['returnClass'];

        $coreModules = array_flip($this->coreModules);
        $enabledModules = array_flip($this->enabledModules);
    
        $modules = [];
        $modulesCount = count($this->modules);
        $modules = array_fill(0, $modulesCount, null);

        foreach ($this->modules as $id => $class) {
            if (!$includeCoreModules && isset($coreModules[$class])) {
                // Skip core modules
                continue;
            }

            if ($enabledOnly && !isset($coreModules[$class]) && !isset($enabledModules[$id])) {
